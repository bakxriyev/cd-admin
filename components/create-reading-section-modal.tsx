...
interface CreateReadingSectionModalProps {
  isOpen: boolean
  onClose: () => void
  readingId: string
...
export function CreateReadingSectionModal({
  isOpen,
  onClose,
  readingId,
  onSectionCreated,
  editingSection,
}: CreateReadingSectionModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      sectionData.append("reading_id", readingId)
      ...
      if (editingSection) {
        await api.readingQuestions.update(editingSection.id.toString(), sectionData)
      } else {
        await api.readingQuestions.create(sectionData)
      }
      ...
      console.error("Failed to save reading section:", error)
      ...
      <DialogContent className="bg-slate-800 border-slate-700 text-white max-w-xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-white flex items-center gap-2">
            ...
            {editingSection ? "Reading Bo'limini Tahrirlash" : "Reading Bo'limi Qo'shish"}
          </DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              placeholder="Group instruction text"
              rows={2}
            />
          </div>
